<%@taglib uri="http://java.sun.com/jsp/jstl/core"  prefix="c" %>
<%@page contentType="text/html" pageEncoding="UTF-8" import="java.sql.*,java.util.*,javazoom.upload.*"%>
<%@page import="java.sql.*"%>
<%@page import="java.sql.DriverManager"%>
<%@page import="java.sql.Connection"%>
<%@page import="javax.servlet.http.HttpServlet"%>
<%@page import="javax.servlet.http.HttpServletRequest"%>
<%@page import="javax.servlet.http.HttpServletResponse"%>
<%@page import="javax.servlet.http.*"%>
<%
    try {
    MultipartFormDataRequest mrequest = new MultipartFormDataRequest(request); 
    String s1=mrequest.getParameter("t1");
    String s2=mrequest.getParameter("t2");
    String s3=mrequest.getParameter("t3");
     HttpSession hs=request.getSession();
    String s9=  hs.getAttribute("log").toString(); 
        if((s1!=null) && ( s2!=null)&&(s3!=null))      
        {
            UploadBean ub=new UploadBean();
           ub.setFolderstore(getServletContext().getRealPath("")+"//photo//");
            Hashtable ht=mrequest.getFiles();
           
            UploadFile uf=(UploadFile)ht.get("t4");
            uf.getFileSize();
            ub.store(mrequest,"t4");
            
            Class.forName("com.mysql.jdbc.Driver");
                Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/project","root","");
                Statement stmt=con.createStatement();
                int count=stmt.executeUpdate("insert into globle values("+s9+",'"+s1+"','"+s2+"','"+s3+"') ");

             if(count>0)
                {
                                response.sendRedirect("Globle.jsp");
                }
                else
                {
                                System.out.println("Some Technical Problem Occured.");	
                }
        
                con.close();  
        
        }
    }
    catch(Exception e)
    {
        
    }
    %>